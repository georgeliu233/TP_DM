...
class TFBertSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if self.config.use_decoder:
            _,_,a,b = attention_scores.get_shape().as_list()
            decoder_mask = tf.reshape(tf.matrix_band_part(tf.cast(tf.ones([a,b]),tf.float32)),[1,1,a,b])
            attention_scores = attention_scores * decoder_mask - 1e4*(1-decoder_mask)
